...
-- Prompt 4: Count the number of unique users per day who logged in from either a mobile device or web.
...
-- Prompt 12: As a data scientist at Amazon Prime Video, you are tasked with enhancing the in-flight entertainment experience for Amazon's airline partners.
...
-- Prompt 16: Determine whether hosts or guests leave higher review scores on average.
...
-- Prompt 17: Find the details of each customer regardless of whether the customer made an order.
...
-- Prompt 18: Find how many reviews exist for each review score given to 'Hotel Arena'.
...
-- Prompt 19: Find the number of reviews received by Lo-Lo's Chicken & Waffles for each star. 
...
-- Prompt 20: Find employees who started in June and have even-numbered employee IDs.
SELECT *
FROM worker
...
-- Prompt 21: Find employees who started in February and have odd-numbered employee IDs.
SELECT *
FROM worker
...
-- Prompt 22: Find wine varieties tasted by 'Roger Voss' and with a value in the 'region_1' column of the dataset. Output unique variety names only.
...
-- Prompt 23: Find the total costs and total customers acquired in each year.
...
-- Prompt 24: Find the average cost of each request status. Request status can be either 'success' or 'fail'.
...
-- Prompt 25: Find the hour with the highest gasoline cost. Assume there's only 1 hour with the highest gas cost.
SELECT hour
FROM lyft_rides
GROUP BY hour
...
-- Prompt 26: Find the gender that has made the most number of doctor appointments.
...
-- Prompt 27: Find the number of US-based wineries that have expensive wines. A wine is considered to be expensive if its price is $200 or more.
...
-- Prompt 28: Find the total number of housing units completed for each year.
...
-- Prompt 29: Find the total number of records that belong to each variety in the dataset.
...
-- Prompt 31: Find the total AdWords earnings for each business type.
...
-- Prompt 32: Find the number of entries per star.
...
-- Prompt 33: Find the total number of searches for houses in Westlake neighborhood with a TV among the amenities.
...
-- Prompt 34: Find the number of relationships that user  with id == 1 is not part of.
...
-- Prompt 35: Find all the songs that were top-ranked (at first position) at least once since the year 2005.
SELECT DISTINCT
    song_name
...
-- Prompt 36: Find the number of Yelp businesses that sell pizza.
SELECT
...
-- Prompt 37: Find doctors with the last name of 'Johnson' in the employee list.
...
-- Prompt 38: Find all Lyft rides which happened on rainy days before noon.
SELECT *
FROM lyft_rides
WHERE weather = 'rainy'
    AND hour < 12;
...
-- Prompt 39: Find all Lyft drivers who earn either equal to or less than 30k USD or equal to or more than 70k USD.
...
-- Prompt 40: Find the advertising channel(s) where Uber spent more than 100k USD in 2019.
...
-- Prompt 41: Find songs that are ranked between 8-10.
...
-- Prompt 42: Find the year that Uber acquired more than 2000 customers through advertising using celebrities.
SELECT year
FROM uber_advertising
...
-- Prompt 43: Find the number of 5-star reviews earned by Lo-Lo's Chicken & Waffles.
...
-- Prompt 45: What is the total sales revenue of Samantha and Lisa?
SELECT
